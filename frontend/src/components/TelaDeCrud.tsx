...
function TelaDeCrud(props){
    ...
    if(props.layout===0) {
        ...
        return(props.loading?<div className={"w-full h-full flex items-center justify-center"}> <h1 className={"text-[50px]"}>Carregando Contatos...</h1></div>:<div className={"bg-gray-50 shadow-lg h-full p-10 flex flex-col gap-2"}>
            <div><button className={"rounded-full border hover:bg-red-400"} onClick={()=>{
                props.setLayout(0)
                setEditar(false)
                setId(-1)
                setPage(1)
                ...
                setSort("none")
                setOrder("none")
                setProcurar(false)}}><X></X></button></div>
            <div className={"grid grid-cols-2 w-full"}>
                <div className={"col-span-1"}><h1 className={"text-black text-[30px]"}>Contatos</h1></div>
                <div className={"col-span-1 flex justify-end items-center w-full gap-3"}>
                    <div className={" flex col-span-1 ml-3 items-center"}>{procurar?<div className={"flex items-center gap-1"}><input placeholder={"Nome/email de busca"} className={"rounded-full border text-center"} onChange={(e)=>{q=e.target.value}}/>
                        <button
                    ...
                    </button></div>:<button onClick={()=>{setProcurar(true)}}><Search className={"hover:size-7"}></Search></button>}</div>
                    <button className={"items-center"} onClick={()=>{
                        ...
                        setSort("none")
                        setOrder("none")
                        setProcurar(false)}}><RefreshCw className={"hover:size-7"}></RefreshCw></button>
                    <p className={""}>
                    <button className="
...
" onClick={()=>props.setLayout(2)}>
        ...
        <div className={" rounded-md flex flex-col border border-gray-200 shadow-lg"}>

            <div className={'flex-grow flex-wrap rounded-r-md sm:gap-10 md:gap-30 lg:gap-40 xl:gap-43 2xl:gap-75 rounded-l-md rounded-t-md rounded-b-none flex items-end '}>
            ...
            <div className={'bg-white shadow-lg rounded-b-none flex h-fit'}>
                <ul>
                {
                        ...
                        return(<li className="py-2 text-gray-900 grid grid-cols-5 border-b border-gray-200 xl:gap-30 2xl:gap-45 ">
                            <div><p className="font-bold ml-7">{contact.nome}</p></div>
                            <div><p className="text-sm text-gray-600">
                                {contact.email}
                            </p></div>
                            <div ><p className="text-sm text-gray-600">{contact.phone}</p></div>
                            <div ><p className="text-sm text-gray-600">{contact.createdAt.slice(0,19)}</p></div>
            ...
            <div className={'rounded-r-md rounded-l-md rounded-b-md rounded-tl-none rounded-tr-none grid grid-cols-2 gap-2'}>
                <div className={"flex flex-row gap-1"}><p className={"mt-2"}>Order By:</p> <select
                    id="ordenacao"
                    name="ordenacao"
                    className="
        ...
        "
                    ...
                    onChange={(e)=>{
                        ...
                        props.getContacts("",page,pageSize,sortaux,order==="none"?"asc":order)}
                    }
                >
        ...
        "
            ...
            sm:text-sm p-2"
                    ...
                    <div className={"mr-3"}><button
                        ...
                        // @ts-ignore
                        props.getContacts("",1,pageSizeAux===''?10:pageSizeAux,sort,order)
                        setPage(1)
                    }}>
                        <Check size={18}/>
                    ...
                    <div className={"flex gap-2 mr-3"}>
                        <ChevronLeft className={"mt-2 border rounded-full hover:size-7"} onClick={()=>{
                            if(page-1!==0) {
                                ...
                                // @ts-ignore
                                props.getContacts("", newPage, pageSize===''?10:pageSize,sort,order)
                            }
                        }}></ChevronLeft>
                        <p className={"mt-2"}>{page}</p>
                        <ChevronRight className={"mt-2 border rounded-full hover:size-7"} onClick={ async ()=>{
                            const newPage = page + 1
            ...
            <div className={"flex flex-col w-full h-full"}>
                ...
                <div className={" w-full h-[300px] bg-gray-100 border-gray-200 shadow-lg rounded-md"}>
                    <div className={"grid grid-rows-3 mt-13 gap-3"}>
                        <div className={"justify-center flex flex-col items-center"}><h1 className={'text-center h-[22px]'}>Nome</h1><input className={"bg-gray-300 rounded-md border text-center w-[250px]"} onChange={(e)=>contatoaux.nome=e.target.value} /></div>

                        <div className={"justify-center flex flex-col items-center"}><h1 className={'text-center h-[22px]'}>Email</h1><input className={"bg-gray-300 rounded-md border text-center w-[250px]"}  onChange={(e)=>contatoaux.email=e.target.value}/></div>

                        <div className={"justify-center flex flex-col items-center"}><h1 className={'text-center h-[22px]'}>Phone</h1><input className={"bg-gray-300 rounded-md border text-center w-[250px]"}  onChange={(e)=>contatoaux.phone=e.target.value}/></div>
                    </div>
                    <div className={'grid grid-cols-2 justify-center items-center '}>
                        <div className={'flex justify-end mr-25'}>
                        <button className={'rounded-md border hover:bg-red-400'} onClick={()=>{
                            props.setLayout(1);
                        }}><X></X></button></div>
                        <div>
                        <button
...
" onClick={()=>props.setLayout(2)}>
            ...
            <div className={"border-gray-200 shadow-lg grid grid-rows-15 border"}>
                ...
                <div className={'flex-grow flex-wrap row-span-3 bg-gray-100 shadow-lg rounded-r-md gap-10 sm:gap-15 md:gap-40 lg:gap-55 xl:gap-60 2xl:gap-90  rounded-l-md rounded-t-md rounded-b-none flex items-end '}>
...

                        {
                            props.contacts.map((contact:Contact) =>{
                                 if(editar && contact.id===id){
                                    return(<li className="py-2 text-gray-900 grid grid-cols-4">
